...
export default function ProgramCard({ title, time, description, image }: ProgramCardProps) {
  return (
    <Card className="overflow-hidden hover-elevate transition-transform duration-300 hover:scale-105" data-testid={`card-program-${title.toLowerCase().replace(/\s+/g, '-')}`}>
      ...
      <div className="p-6">
        ...
        <p className="text-sm text-muted-foreground" data-testid="text-program-desc">{description}</p>
      </div>
    </Card>
  );
}
